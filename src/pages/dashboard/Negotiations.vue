    ...
    <DefaultNav>
        <div class="big-container">
            <div class="left" v-if="!product">
                ...
                <div class="messages">
                    <a @click="openConversation(conversation)" v-for="conversation, index in conversations" :key="index"
                        href="javascript:void(0)" class="each-message">
                        <div class="left-message">
                            <h3>{{(participant(conversation).first_name + " " + participant(conversation).last_name)}}
                            </h3>
                            <p>{{ conversation.crop.title }}</p>
                        </div>
                        <div class="right-message">
                            <p>{{ lastMessageData(conversation).dateString }}, {{ lastMessageData(conversation).time }}
                            </p>
            ...
            <div class="right" v-if="product">
                <ChatView :sender="userData.user" :recepient="product.user" :title="product.user.id != userData.user_id
                    ? product.user.first_name + ' ' + product.user.last_name
                    : participant(conversationData).first_name + ' ' + participant(conversationData).last_name
                " :messages="conversationData.negotiations" :loadMessages="getNegotiation"
                    :onSendMessage="sendNegotiationMessage" :onSendOffer="sendNegotiationOffer"
                    :onAcceptOffer="acceptNegotiationOffer" :onDeclineOffer="declineNegotiationOffer" />
                <div class="right-top-section">
                    <h1 v-if="product.user.id != userData.user_id">{{
                        product.user.first_name + " " +
                            product.user.last_name
                    }}
                    </h1>
                    <h1 v-else>{{
                        participant(conversationData).first_name + " " +
