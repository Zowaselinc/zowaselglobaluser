    ...
    <div class="big-container">
        ...
        <div class="messages">
          ...
          >
            <div class="left-message">
              <h3>
                {{
                  participant(conversation).first_name +
                  " " +
                  participant(conversation).last_name
                }}
              </h3>
            ...
            <div class="right-message">
              <p>
                {{ lastMessageData(conversation).dateString }},
                {{ lastMessageData(conversation).time }}
              </p>
      ...
      <div v-if="product" class="right">
        <ChatView
          ...
          :title="
            product.user.id != userData.user_id
...
export default {
  ...
  methods: {
    participant(conversation) {
      return conversation.initiator.id == this.userData.user_id
    ...
    getConversations() {
      MarketPlaceService.getNegotiationsByUser(
    ...
    acceptNegotiationOffer(message) {
      MarketPlaceService.acceptNegotiationOffer(message.id, (response) => {
        this.$router.push(
          `/dashboard/marketplace/order/${response.data.order.order_hash}`
        );
      });
    },
