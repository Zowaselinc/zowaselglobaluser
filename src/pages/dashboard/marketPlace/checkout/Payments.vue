  ...
  methods: {
    ...
    makePayment() {
            ...
            TransactionService.verifyTransaction(
              {
                ...
                ...(vm.paymentPercent ||
                vm.order.payment_status == "PARTIALLY_PAID"
                  ? { partial: true }
                  : {}),
              },
              (response) => {
                if (!response.error) {
                  vm.$router.push(
                    `/dashboard/marketplace/confirmpayments/${vm.order.order_hash}`
                  );
                }
              }
            );
      ...
      var detectWindow = setInterval(() => {
        ...
        if (checkout) {
          document
            .querySelector(".transaction-sumary-container .left-container")
            .appendChild(checkout);
